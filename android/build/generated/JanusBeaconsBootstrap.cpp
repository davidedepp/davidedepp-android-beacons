...
#define TAG "com.alfonsojanus.beacons"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> JanusBeacons_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("JanusBeacons.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::JanusBeaconsBindings::lookupGeneratedInit(
...
static void JanusBeacons_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(JanusBeacons_getBinding)->GetFunction());
}

static void JanusBeacons_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::JanusBeaconsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry JanusBeaconsBinding = {
	"com.alfonsojanus.beacons",
	JanusBeacons_init,
	JanusBeacons_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_alfonsojanus_beacons_JanusBeaconsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.alfonsojanus.beacons", &JanusBeaconsBinding);
	titanium::KrollBindings::addExternalLookup(&(::JanusBeaconsBindings::lookupGeneratedInit));
}
